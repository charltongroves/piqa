r"""Structual Similary-based Index (SSI)

This module implements the SSI in PyTorch.

Wikipedia:
    https://en.wikipedia.org/wiki/Salience_(neuroscience)#Visual_saliency_modeling

Credits:
    Inspired by the [official implementation](https://sse.tongji.edu.cn/linzhang/IQA/SSI/SSI.htm)

References:
    [1] SSI: A Structual Similary-Induced Index for Perceptual Image Quality Assessment
    (Zhang et al., 2014)
...
def ssi(
    ...
    r"""Returns the SSI between \(x\) and \(y\),
    without downsampling and color space conversion.

    Args:
        x: An input tensor, \((N, 3, H, W)\).
        y: A target tensor, \((N, 3, H, W)\).
        vs_x: The input Structual Similary, \((N, H, W)\).
        vs_y: The target Structual Similary, \((N, H, W)\).
    ...
    # Structual Similary-based Index
    s = s_vs * s_g ** alpha
    ssi = (s * vs_m).sum(dim=(-1, -2)) / vs_m.sum(dim=(-1, -2))
...
def sdsp(
    ...
    Returns:
        The Structual Similary tensor, \((N, H, W)\).
...
class SSI(nn.Module):
    r"""Creates a criterion that measures the SSI
    between an input and a target.

    Before applying `ssi`, the input and target are converted from
    RBG to LMN and downsampled by a factor \( \frac{\min(H, W)}{256} \).

    The Structual Similary maps of the input and target are determined by `sdsp`.
    ...
    def forward(
        ...
        # SSI
        l = ssi(input, target, vs_input, vs_target, kernel=self.kernel, **self.kwargs)
